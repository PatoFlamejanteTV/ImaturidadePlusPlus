...
function onEvent(name, value1, value2)
	if name == 'SetWindowPos' then
		...
		elseif value1 == 'defScaleY' then
			...
			if useDefScale then
				setPropertyFromClass('openfl.Lib', 'application.window.x', screenX * (value1/100) - defScaleX / 2)
				setPropertyFromClass('openfl.Lib', 'application.window.y', screenY * (value2/100) - defScaleY / 2)
			else
				setPropertyFromClass('openfl.Lib', 'application.window.x', screenX * (value1/100) - getPropertyFromClass('openfl.Lib', 'application.window.width') / 2)
				setPropertyFromClass('openfl.Lib', 'application.window.y', screenY * (value2/100) - getPropertyFromClass('openfl.Lib', 'application.window.height') / 2)
			end
		end
	end
end
