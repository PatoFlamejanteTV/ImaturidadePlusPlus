function onCreate()
	...
	for i = 0, getProperty('unspawnNotes.length')-1 do
		...
		if getPropertyFromGroup('unspawnNotes', i, 'noteType') == 'Bullet Note' then
			...
			setPropertyFromGroup('unspawnNotes', i, 'multSpeed', 1.0); --Change note speed (holy shit 0.6 is good)
			setPropertyFromGroup('unspawnNotes', i, 'missHealth', 10.0); --Change amount of health to take when you miss like a fucking moron
		end
	end
...
function goodNoteHit(id, noteData, noteType, isSustainNote)
	if noteType == 'Bullet Note' then
		...
		playSound('gunshot', 1.0, 'shot');
		soundFadeOut('shot', 0.0, 0);
...
function noteMiss(id, noteData, noteType, isSustainNote)
	if noteType == 'Bullet Note' then
		playSound('gunshotPierce', 1.0, 'shotmiss');
